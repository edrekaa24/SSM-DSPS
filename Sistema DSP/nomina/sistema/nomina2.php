...
$sql = "SELECT TOTXPER.*, Nomina.nomina,EMPLEADOS.FECINIC FROM TOTXPER,Nomina,EMPLEADOS WHERE EMPLEADO LIKE '".$rfc."' AND TOTXPER.STADO NOT IN ('PP') and TOTXPER.TIPAG<>0 and nomina.siglas=TOTXPER.ref_nom AND EMPLEADOS.AFILIACION=TOTXPER.EMPLEADO ORDER BY ANO DESC , QUINCENA DESC";
            ...
            <div class="maincontentinner">
                ...
                if(empty($rfc))
                {
                 ...
                 ?>    
                            ...
                            while (($datos = mssql_fetch_array($stmt, MSSQL_BOTH))) {
                                ...
                                <h4><b>FECHA DE INGRESO A LA DEPENDENCIA:        </b><?php echo $fechini;?></h4><br/><br/>
                ...
                <?php
                    ...
                    while (($row = mssql_fetch_array($stmt2, MSSQL_BOTH))) {
                          ...
                          $plaza=$row['UNIDAD'].$row['ACTIVIDAD'].$row['PROYECTO'].$row['PARTIDA'].$row['PUESTO'].$row['NUMPUES'].$row['GRUPO'].$row['FUNCION'].$row['SUBFUNCION'];
                          $plaza=str_replace(' ', '', $plaza);
                        ...
                        $selected2 = mssql_select_db('COMUN', $link2) or die("Algo fue mal mientras se conectaba a MSSQL"); 
                          $prod='%'.rtrim($tipo_pagp).'%';
                        $sql2 = "SELECT * FROM dsp WHERE unidad='$ur' and productos like ('$prod')";
                        
                         $stmt3 = mssql_query($sql2);
                            if( !mssql_num_rows($stmt3)) {
                                $dsp="No hay resultados";
                            }
                        ...
                        ?>
                            ...
                            <td> <a href="nomina3.php?id=<?php echo $id;?>&rfc=<?php echo $rfc;?>&nombre_emp=<?php echo $nombre_emp;?>&curp=<?php echo $curp;?>&puesto=<?php echo $puesto;?>&cve_centro=<?php echo $cve_centro;?>&centro=<?php echo $centro;?>&nomina=<?php echo $nomina;?>&cuenta=<?php echo $cuenta;?>&fecha=<?php echo $fechini;?>&qna=<?php echo $qna;?>&pdf=<?php echo $pdf4;?>"> <img src="images/eye.png" style="width:30px;height:18px;border:0;"></a></td>
                            </TR>  
                            <?php
                            }
                            }
