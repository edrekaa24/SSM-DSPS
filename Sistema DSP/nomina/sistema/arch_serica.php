...
switch ($tipo) {
	case 'O':
		$tipag="AND A.TIPAG IN ('1')";
		break;
	case 'ER':
		$tipag="AND A.TIPAG IN ('2','6')";
		break;
	case 'D':
		$tipag="AND A.NOMPROD LIKE ('PRDC%')";
		break;
	}
...
//query a mssql
 $sql = "SELECT 'D' AS TIPO, 
			...
			13411 AS P_13411,
			'MR' AS C_MR,
			sum(IIF(D.concepto='16',importe,0)) as '13411',
			15403 AS P_15403,
			'' AS C_X7,
			0 AS '15403',
			15402 AS P_15402,
			...
			10002 AS P_10002,
			'38' AS C_38,
			sum(IIF(D.concepto='38',importe,0)) as '10002',
			20001 AS P_20001,
			'OD' AS C_OD,
			...
			20007 AS P_20007,
			'17' AS C_17,
			sum(IIF(D.tipo+D.concepto='17',importe,0)) as '20007',
			20008 AS P_20008,
			'17' AS C_X17,
			...
			AND A.EMPLEADO IN (SELECT distinct EMPLEADO FROM MOVXPER WHERE CONCEPTO='02' AND TIPO='2' AND ANO='$anio' AND QUINCENA='$qna' AND REFERENCIA in ($ref))
			GROUP BY 
			B.CEDULA,
			A.NOMBRES,
			A.AP_PATERNO,
