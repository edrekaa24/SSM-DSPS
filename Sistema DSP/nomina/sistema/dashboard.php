...
if(isset($_COOKIE["misitio_userid"]))
    ...
    header('Location:index.php');
$datos="select max(quincena) as quincena from totxper where ano=(select max(ano) from totxper)";
$dat=mssql_query($datos,$link);
while (($r = mssql_fetch_array($dat, MSSQL_BOTH))) {                       
        $qna=$r['quincena']-1;
    }

$datos1="select max(ano) as ano from totxper";
$dat1=mssql_query($datos1,$link);
while (($r1 = mssql_fetch_array($dat1, MSSQL_BOTH))) {                       
        $ano=$r1['ano'];
    }
$sql="select sum(percepciones)-sum(deducciones) as total from totxper where ano='".$ano."' and quincena='".$qna."' and stado='A'";
            ...
            <div class="maincontentinner">
                <div class="row-fluid">
                    <div id="dashboard-left" class="span8">
                        <h5 class="subtitle">Estadísticas</h5>
                        <ul class="shortcuts">
                            <li class="events">
                                <a href="">
                                    <span class="shortcuts-icon iconsi-event"></span>
                                    <span class="shortcuts-label">Quincena <?php echo $qna;?>-<?php echo $ano;?></span>
                                </a>
                            </li>
                            <li class="products">
                                <a href="">
                                   ...
                                   <span class="shortcuts-icon iconsi-cart"></span>
                                    <span class="shortcuts-label"><?php echo money_format('%(#10n',$total);?></span>
                                </a>
                            </li>
                            <li class="archive">
                                <a href="reportes.php">
                                    <span class="shortcuts-icon iconsi-archive"></span>
                                    <span class="shortcuts-label">Archivos</span>
                                </a>
                            </li>
                            <li class="help">
                                <a href="">
                                    <span class="shortcuts-icon iconsi-help"></span>
                                    <span class="shortcuts-label">Ayuda</span>
                                </a>
                            </li>
                        </ul>
                        <br/>
